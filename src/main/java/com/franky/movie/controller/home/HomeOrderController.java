...
public class HomeOrderController {
	...
	public Result<Boolean> orderPay(@RequestParam(name="order_sn",required=true)String sn,
									@RequestParam(name="payway",required=true)String paymode){
		Order order = orderService.find(sn);
		...
		if(paymode==null||"".equals(paymode)){
			return Result.error(CodeMsg.HOME_ORDER_PAYMODE_ERROR);
		}
		order.setPaymode(paymode);//设定支付方式
		long passTime = System.currentTimeMillis()/1000 - order.getCreateTime().getTime()/1000;
		if(passTime > orderTimeout){
			return Result.error(CodeMsg.HOME_ORDER_STATUS_TIMEOUT);
		}
		if(paymode!=null&&paymode.equals("0")){
		...
		if(paymode!=null&&paymode.equals("1")) {
			if (order.getAccount().getScore()<order.getNewMoney().intValue()) {
				return Result.error(CodeMsg.HOME_ORDER_SCORE_ERROR);
			}
		}
		//订单状态都合适
